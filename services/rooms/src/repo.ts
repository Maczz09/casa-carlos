...
export class RoomsRepo {
  ...
  async insertCategory(row: CategoryRow, atributoIds: string[]): Promise<Category> {
    ...
    return { id: row.id, nombre: row.nombre, descripcion: row.descripcion, camas: row.camas, atributoIds, activo: row.activo };
  }

  async listCategories(): Promise<Category[]> {
    const categorias = await this.db.select().from(schema.roomsCategorias).where(eq(schema.roomsCategorias.activo, true)).all();
